...
编辑区与内联草稿行统一使用**多行自适应输入**：`TextWrapping=Wrap` + `AcceptsReturn=True`、不设上限，高度随内容增长；Enter 提交、Shift+Enter 换行（`PreviewKeyDown` 拦截 Enter 而不拦截 Shift+Enter）、Esc 取消、失焦提交。两者共用 `Todo.RowEditor` 样式，走**无框行内编辑**：透明底、`BorderThickness=0,0,0,1`（仅一条灰色细线表示"正在输入"，取 `Todo.SeparatorBrush`）、`Padding=0`、13px，并由行宿主套一层悬停档底色，使编辑态仍像原来的待办行而不是突然冒出一个表单输入框；进入编辑时隐藏悬停操作区（编辑分支在 `BuildRow` 里提前 `return`，根本不构建操作区），光标落在**文本末尾**而不是全选（`CaretIndex = Text.Length`，改已有待办时很少要整体覆盖）。

对齐是量出来的，不是估的：全局 TextBox 的文字有约 2px 内部内缩，因此编辑框与草稿行都取 `Margin=-2,0,6,0` 抵消它；模板里的占位提示是普通 `TextBlock`、没有这个内缩，故单独给 `Margin=2,0,0,0`，否则输入时占位文字与真正输入的文字会错开。离屏渲染后按像素扫描验收：普通行 / 编辑态 / 草稿行的文字起始列都是 39，位移 **0 px**（早期版本编辑态右移 2px、且整体比普通行低 9px 并带填充底与全边框，这也是"不自然"的来源）。草稿行在行内样式下空框不可见，因此保留占位提示（`Tag="添加待办…"`）。

底部不再有固定新增行：点击标题栏的加号后，由 `Render()` 在**普通区开头**（`visible.Count(i => i.IsPinned && !i.IsCompleted)` 计算出的槽位）插入一个 `BuildDraftRow()` 生成的内联草稿行，即新事项提交后所在的位置，因此提交时不会发生位置跳变；草稿行不入 `_rowHosts`，不参与拖动与排序计算。
...
弹出快捷键对「快捷键隐藏」和边缘隐藏状态执行恢复并激活窗口；再次按下时用 `Hide()` 收起，**不用 `WindowState=Minimized`**——本窗口是无边框 + `AllowsTransparency` + `ShowInTaskbar=False`，走系统最小化不会进任务栏，会在桌面留下一条裁切横条。因此新增独立状态 `_hotkeyHidden`（与停靠的边缘隐藏互不影响，恢复时各自复位）。设置窗口或删除确认气泡打开时忽略该按键：它们是独立顶层窗口，收起主窗口会把浮层悬空留下。窗口是否始终置顶由设置项 `AlwaysOnTop` 决定（默认关闭），保存后立即作用于 `Window.Topmost` 并随状态持久化。
