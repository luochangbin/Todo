...
编辑区与内联草稿行统一使用**多行自适应输入**：`TextWrapping=Wrap` + `AcceptsReturn=True`、不设上限，高度随内容增长；Enter 提交、Shift+Enter 换行（`PreviewKeyDown` 拦截 Enter 而不拦截 Shift+Enter）、Esc 取消、失焦提交。两者共用 `Todo.RowEditor` 样式，走**无框行内编辑**：透明底、`BorderThickness=0,0,0,1`（仅一条强调色细线表示"正在输入"）、`Padding=0`、13px，并由行宿主套一层悬停档底色，使编辑态仍像原来的待办行而不是突然冒出一个表单输入框；进入编辑时隐藏悬停操作区（编辑分支在 `BuildRow` 里提前 `return`，根本不构建操作区），光标落在**文本末尾**而不是全选（`CaretIndex = Text.Length`，改已有待办时很少要整体覆盖）。
